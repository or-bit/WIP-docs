...
\begin{itemize}
	...
	\item \method{+ redirectToNewOreder(): void}: renderizza e si sposta sulla pagina di costruzione dell'ordine;
	...
	\item \method{+ reloadTotal(): double}: ricalcola il totale e ne restituisce il valore;
...
\setclass{BubbleAndEat::Chef::ChefBubble}
...
\begin{itemize}
	...
	\end{itemize}
	\item \method{+ componentDidMount(): void}: viene invocata alla creazione della classe e invoca la funzione connect;
	\item \method{+ connect():void}: connette la classe al resto dell'applicazione tramite socket e invoca fetchOrders;
	\item \method{+ fetchOrders():void}: invia la richiesta di visualizzazione delle ordinazioni;
	\item \method{+ disconnect(): void}: libera le risorse quando viene distrutta la classe;
	\item \method{+ markOrdinationCOmpleted(id:string): void}: indica come completata un'ordinazione;
		\begin{itemize}
			\item \param{id:string}: identifica l'ordine da completare;
		\end{itemize}
	\item \method{+ render(): void}: renderizza il componente.
\end{itemize}


\setclass{BubbleAndEat::Manager::BubbleManager}
\paragraph[::BubbleManager]{\class}\mbox{}\\ \label{\class}
\begin{figure}[H]
	\centering
%	\includegraphics[width=7cm]{./diagrammi/demo/client/manager/bubblemanager.png}
	\caption{Classe \class}
\end{figure}
\textbf{Descrizione:}\\
...
\begin{itemize}
	...
	\item \field{- formDataPrice: price}: valore contenuto nel campo \virgolette{Price} di creazione e modifica di una pietanza del menu;
...
\begin{itemize}
	...
	\item \method{+ redirectToFormEdit():void}: invoca la renderizzazione della 
	\item \method{+ redirectToHome():void}: invoca la renderizzazione della 
	\item \method{+ redirectToMenu():void}: invoca la renderizzazione della 
	\item \method{+ redirectToOrders():void}: invoca la renderizzazione della 
	\item \method{+ render():void}: invoca la renderizzazione della 
