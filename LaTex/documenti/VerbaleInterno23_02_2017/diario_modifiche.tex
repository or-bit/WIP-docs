\section*{Diario delle modifiche}


\bgroup
\begin{longtable}{|>{\centering}P{0.9cm}|>{\centering}P{1.7cm}|>{\centering}P{2.9cm}|>{\centering}P{2.5cm}|P{4cm}|}
	\hline \multicolumn{1}{|c|}{\textbf{Versione}} & \multicolumn{1}{c|}{\textbf{Data}} & \multicolumn{1}{c|}{\textbf{Persona}} & \multicolumn{1}{c|}{\textbf{Ruolo}} & \multicolumn{1}{c|}{\textbf{Descrizione}} \\ \hline  
	\endfirsthead 
	
	\hline \multicolumn{1}{|c|}{\textbf{Versione}} & \multicolumn{1}{c|}{\textbf{Data}} & \multicolumn{1}{c|}{\textbf{Persona}} & \multicolumn{1}{c|}{\textbf{Ruolo}} & \multicolumn{1}{c|}{\textbf{Descrizione}} \\ \hline  
	\endhead 
	
	\hline \multicolumn{5}{|r|}{{Continua nella prossima pagina}} \\ \hline 
	\endfoot 
	
	\hline \hline 
	\endlastfoot 
	
	% IN ORDINE DALLA MODIFICA PIÙ RECENTE ALLA PIÙ VECCHIA 
	\hline 1.0.0 & 01-03-2017 & \nick & \Responsabile & Approvazione del documento \\
	\hline 0.2.0 & 01-03-2017 & Lorenzo Ferrarin & \Verificatore & Verifica del documento \\	
	\hline 0.1.1 & 28-02-2017 & Alice V. Sasso & Analista & Correzione degli errori segnalati. \\
	\hline 0.1.0 & 28-02-2017 & \nick & \Verificatore & Verifica del documento. Segnalati errori di digitazione. \\ 
	\hline 0.0.1 & 23-02-2017 & Alice V. Sasso & Analista & Creazione e stesura documento \\
	\hline 
\end{longtable}
\egroup
